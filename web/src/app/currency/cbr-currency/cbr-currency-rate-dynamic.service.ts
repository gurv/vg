...
export class CbrCurrencyRateDynamicService {
  ...
  getCurrencyRates(currency: CbrCurrency,
    ...
    searchParams.set('date_req1', this.dateToString(beginDate));
    searchParams.set('date_req2', this.dateToString(endDate));
    searchParams.set('VAL_NM_RQ', currency.id);

    return this.http.get(this.url, {search : searchParams})
      .map((response: Response) => this.parseXml(response.text()))
      .do((data: CbrCurrencyRate[]) => console.debug("Подготовлен список курсов Банка России валюты", currency.isoCharCode, "в количестве", data.length, "шт."))
  }
